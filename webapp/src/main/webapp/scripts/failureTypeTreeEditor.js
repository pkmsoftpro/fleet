...
var FAILURE_TREE_VARS = {
    ...
    ELEMENT_CLASSES : {
        ...
        FAILURE_CAUSE_HOLDER : "failureCauseHolder",
...
dojo.declare("twms.failureTree.TreeDataManager", null, {
     ...
     */
    getPeerNodeDefinitionIds : function(/*Node (treeNode [instanceOf = 'FailureType'/'FailureCause'])*/ node) {
        if(node.instanceOf != FAILURE_TREE_VARS.CONSTANTS.FAILURE_CAUSE && node.instanceOf != FAILURE_TREE_VARS.CONSTANTS.FAILURE_TYPE) {
    ...
    _findPeerNodes : function(/*Tree Fragment*/ node, /*tree dataStructure*/ treeFragment) {
        ...
        return (node.instanceOf === FAILURE_TREE_VARS.CONSTANTS.FAILURE_TYPE) ?
               parentNode.failureTypeChildren : parentNode.failureCauseChildren;
    },
    ...
    _isChildPresent : function(/*Tree Fragment*/ child, /*Tree Fragment*/ testParent) {
        var childrenList = (child.instanceOf === FAILURE_TREE_VARS.CONSTANTS.FAILURE_TYPE) ?
                           testParent.failureTypeChildren : testParent.failureCauseChildren;
            return childrenList &&
...
dojo.declare("twms.failureTree.DefinitionCreator", null, {
    ...
    createNewFailureCauseDef : function(/*String*/ label, /*int*/ depth, /*String*/ description, /*Function*/ callback) {
        this._createNewDef(label, depth, description);
        var self = this;
        this._requestCreation = function() {
            twms.failureTree.AJAXer.createFailureCauseDef(this._labelInput.value, this._depthInput.value, this._descInput.value, function(data) {
                self._onCompletion();
                callback(data);
            });
        }
...
dojo.declare("twms.failureTree.NodeAgent", tavant.twms.treeTable.DefaultNodeAgent, {
    ...
    getStub : function(/*String (row type)*/ rowType) {
        if (rowType === FAILURE_TREE_VARS.CONSTANTS.FAILURE_TYPE) {
            return {
                ...
                "nodeType":"leaf",
                "id":0,
                "failureCauseChildren":[]
            };
        } else if (rowType === FAILURE_TREE_VARS.CONSTANTS.FAILURE_CAUSE) {
            return {
                "definition":{"code":"","label":"","id":""},
                "nodeType":"leaf",
                "instanceOf":FAILURE_TREE_VARS.CONSTANTS.FAILURE_CAUSE,
                "id":0
            };
        }
    },
    ...
    getRowInstance : function(/*JSON node*/ nodeObject, /*AbstractParentRow*/ parentRow) {
        ...
        } else if(nodeObject.instanceOf === FAILURE_TREE_VARS.CONSTANTS.FAILURE_CAUSE){
            className = twms.failureTree.FailureCauseRow;
            template = failureCauseRowTemplate;
        }
    ...
    getChildCollections : function(/*JSON node*/ nodeObject) {
        if (nodeObject.instanceOf === FAILURE_TREE_VARS.CONSTANTS.FAILURE_TYPE) {
            return [nodeObject.failureCauseChildren];
...
dojo.declare("twms.failureTree.FailureCauseRow", tavant.twms.treeTable.AbstractRow, {
    ...
    _postCreationCallback : function() {
        this._bindFailureCauseDefEditor();
    ...
    _handleValidationErrors : function(errors) {
        twms.failureTree.FailureCauseRow.superclass._handleValidationErrors.call(this, errors);
        this._validationHandler.markInvalid(errors);
    },

    _clearValidationErrors : function() {
        twms.failureTree.FailureCauseRow.superclass._clearValidationErrors.call(this);
        this._validationHandler.markValid();
    },

    _bindFailureCauseDefEditor : function() {
        var failureCauseDefSpan = getElementByClass(FAILURE_TREE_VARS.ELEMENT_CLASSES.FAILURE_CAUSE_HOLDER, this._domNode);
        var self = this;
        var dataProvider = new twms.failureTree.ComboBoxAjaxDataProvider();
        dataProvider.url = "fetch_failure_causes.action";
        
        var updateView = function() {
            failureCauseDefSpan.innerHTML = self._nodeObject.definition.label;
        };
        ...
        var createNewFailureCauseDefinition = function(/*String (label)*/ label, /*Callback*/ callback) {
            FAILURE_TREE_VARS.definitionCreator.createNewFailureCauseDef(label, self.getDepth(), "", function(newDefData) {
        ...
        var actionObject = new twms.failureTree.ActionObject(this._nodeObject, FAILURE_TREE_VARS.RESOURCE_TYPE.COMBO_BOX,
                failureCauseDefSpan, getNodeDefinition, updateView, setNodeDefinition, createNewFailureCauseDefinition, dataProvider);

        dojo.connect(getExpectedParent(failureCauseDefSpan, "td"), "onclick", function(event) {
...
dojo.declare("twms.failureTree.FailureTypeRow", tavant.twms.treeTable.AbstractParentRow, {
    ...
    _performModelAdd : function(nodeObject) {
        this._nodeObject.failureCauseChildren.push(nodeObject);
    },

    _performModelDrop : function(nodeObject) {
        _findAndRemoveNodeFrm(nodeObject, this._nodeObject.failureCauseChildren);
    },
    ...
    _postCreationCallback : function() {
        ...
        this._failureCauseChildRows = new Array();
        this._validationHandler = new twms.failureTree.ValidationHandler(this);
    ...
    _addChildToDomGraph : function(/*Object [FailureTypeRow/FailureCauseRow]*/ childRow) {
        this._positionAndAddFailureCauseNode(childRow);
    },

    addChild : function(/*Object [FailureTypeRow/FailureCauseRow]*/ row) {
        this._failureCauseChildRows.push(row);
        twms.failureTree.FailureTypeRow.superclass.addChild.call(this, row);
    },

    removeChild : function(/*Object [FailureTypeRow/FailureCauseRow]*/ row) {
        _findAndRemoveRowFrm(row, this._failureCauseChildRows);
        FAILURE_TREE_VARS.resourceManager.releaseHost();
        twms.failureTree.FailureTypeRow.superclass.removeChild.call(this, row);
        row.destroy();
    },

    _positionAndAddFailureCauseNode : function(/*Object [FailureCauseRow]*/ failureCauseChild) {
        dojo.dom.insertAfter(failureCauseChild.getDomNode(), this.getDomNode());
    },
...
dojo.declare("twms.failureTree.ValidationHandler", null, {
    ...
    constructor : function(/*Object [FailureTypeRow/FailureCauseRow]*/ row) {
        this._row = row;
...
dojo.declare("twms.failureTree.ActionObject", null, {
    ...
    constructor : function(/*JSON tree node ("FailureTypw"/"failureCause" instance)*/treeNode,
...
_undefSafeMixin("twms.failureTree.AJAXer", {
    ...
    createFailureCauseDef : function(/*String (label)*/ label, /*int*/ depth, /*String*/ description, /*Function*/ callback) {
        twms.ajax.fireJsonRequest("create_failure_cause.action", {"label" : label , description : description }, callback);
    },
...
function _findAndRemoveNodeFrm(/*JSON node*/ node, /*failureCauseChildren/failureTypeChildren*/ collection) {
...
function _findAndRemoveRowFrm(/*Object [FailureTypeRow/FailureCauseRow]*/ row, /*FailureTypeRow/FailureCauseRow Collection*/ collection) {
    for(var i in collection) {
