...
function convertDate(date){utc=date.getTime()-(date.getTimezoneOffset()*60000);return new Date(utc);}
function parseDate(viewValue){if(!viewValue){ngModel.$setValidity('date',true);return null;}else if(angular.isDate(viewValue)){ngModel.$setValidity('date',true);return convertDate(viewValue);}else if(angular.isString(viewValue)){var date=new Date(viewValue);if(isNaN(date)){ngModel.$setValidity('date',false);return undefined;}else{ngModel.$setValidity('date',true);return convertDate(date);}}else{ngModel.$setValidity('date',false);return undefined;}}
