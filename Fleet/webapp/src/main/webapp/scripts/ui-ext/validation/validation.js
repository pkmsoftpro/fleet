strutsValidator.onErrors = function(input, errors) {
    ...
    if (errors.fieldErrors) {
        for (var fieldName in errors.fieldErrors) {
            dojo.debug("error on field " + fieldName);
            if (form.elements[fieldName] == null) {
                dojo.debug("can't find field " + fieldName + "; ignoring");
                continue;
            }
            if (form.elements[fieldName].touched) {
                for (var i = 0; i < errors.fieldErrors[fieldName].length; i++) {
                    addError(form.elements[fieldName], errors.fieldErrors[fieldName][i]);
                    dojo.debug("added error to field " + fieldName);
                }
            } else {
                dojo.debug(fieldName + " is not touched; so ignoring");
            }
        }
    }
}
...
function clearErrorMessages(form) {
    ...
    for (var i = 0; i < msgs.length; i++) {
        if (msgs[i].getAttribute("type") == "error") {
            dojo.dom.removeNode(msgs[i]);
        }
    }
    dojo.debug("cleared error messages from " + form + " " + form.id);
}
...
function clearErrorLabels(form) {
    ...
    dojo.debug("cleared error labels from " + form + " " + form.id);
}

function addError(e, errorText) {
    dojo.debug("adding error message [" + errorText + "] to " + e + " " + e.id);
