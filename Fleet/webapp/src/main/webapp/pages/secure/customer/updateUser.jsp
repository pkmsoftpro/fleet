     ...
     <table class="form" cellspacing="0" cellpadding="0">
      ...
      <td>
           ...
           <sd:autocompleter id='customerCodeAutoComplete' href='list_fleet_customers.action' name='fleetCustomer' key='%{fleetCustomer.id}' keyName='fleetCustomer.id' 
                ...
                <script type="text/javascript">
                        dojo.addOnLoad(function() {
                            dojo.connect(dijit.byId("customerCodeAutoComplete"), "onChange", function(newValue) {
                                 	//dojo.query('input[id^="customerLocationAutoComplete_"]').forEach(function(text){
                                 		var customer = '<s:property value="%{fleetCustomer.name}"/>' ;
                                 		if(isNaN(customer) && isNaN(newValue) && newValue!= customer){
                                 		  dojo.empty("custBody");
                                 		}                                 		                               	
                              });
                            });
            ...
            <s:hidden name="fleetCustomer" value="%{fleetCustomer.id}"/>                                                              
        </td>
        </tr>
        <tr>
                   ...
                   <script type="text/javascript">
                    ...
                    <td width="4%" style="border:1px solid #EFEBF7;">
                        <u:repeatDelete id="deleter_#myindex" theme="twms">
                            <img id="deletePrice" src="image/remove.gif" border="0" style="cursor: pointer;padding-right:4px;" title="<s:text name="button.common.delete" />"/>
                        </u:repeatDelete>
                    </td>
                </tr>
            </u:repeatTemplate>
...
<tr>
    <td class="non_editable"><s:text name="label.customerUser.firstName"/>:*</td>
...
<tr>
    <td class="non_editable"><s:text name="label.customerUser.lastName"/>:*</td>
...
<tr>
    <td class="non_editable"><s:text name="label.customerUser.email"/>:*</td>
...
<table class="form">
    ...
    <tr>
        <td class="non_editable"><s:text name="label.customerUser.password"/>:*</td>
        <td colspan="3"><s:password maxlength="20" size="30" name="userPassword" /></td>
    </tr>
    <tr>
        <td class="non_editable"><s:text name="label.customerUser.confirmPassword"/>:*</td>
