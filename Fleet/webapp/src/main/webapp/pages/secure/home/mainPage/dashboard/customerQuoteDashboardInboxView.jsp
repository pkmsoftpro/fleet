...
<html>
	<head>
	    <title><s:text name="Customer Quote Dashboard List"/></title>
	    ...
	    <script type="text/javascript">
	        ...
	        function exportToExcel(){
	              ...
	              exportExcel("/customerQuoteDashboards/populateCriteria","listCustomerQuoteDashboardsToExcel.action");
	        }
	        ...
	        function refreshIt() { 
	            publishEvent(SUMMARY_TABLE_UTIL.getRefreshFullTopic("customerQuoteDashboardTable"));
	        }  
	        
	    	function getPageUrl(){
	    		var folderName=document.getElementById("folder").value;
	    		var hasAllChecked=document.getElementById("hasAllChecked").value;
	    		return "listCustomerQuoteDashboards.action?folderName="+folderName+"&hasAllChecked="+hasAllChecked;
	    	}
	    ...
	    <s:hidden id="context" name="context" value="CustomerDashboardQuoteSearch" />
	</head>
	<u:body >
	    <div dojoType="dijit.layout.LayoutContainer" style="width: 100%; height: 100%">
	        <div dojoType="dijit.layout.ContentPane" layoutAlign="top" class="buttonContainer">
	          <u:summaryTableButton id="refreshButton" label="button.common.refresh" onclick="refreshIt" align="right" cssClass="refresh_button" summaryTableId="customerQuoteDashboardTable" />
	        ...
	        <u:stylePicker fileName="SummaryTableButton.css" />
	       	<u:summaryTable eventHandlerClass="tavant.twms.summaryTable.BasicTwmsEventHandler" id="customerQuoteDashboardTable"
                        bodyUrl="customerQuoteDashboardsBody" folderName="${folderName}"
                        detailUrl="customerQuoteDashboardDetail" parentSplitContainerId="split" populateCriteriaDataOn="/customerQuoteDashboards/populateCriteria" extraParamsVar="extraParams">
		            <c:forEach items="${tableHeadData}" var="column">
		               <c:choose>
		               		<c:when test="${column.id=='serialNumber' || column.id=='quoteNumber' || column.id=='serviceRequestNumber' || column.id=='customerLocationName'}">
